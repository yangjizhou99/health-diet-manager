...
**综合报告撰写准则（合并深度分析）：**
- **五维健康矩阵洞察**：脚本生成的 markdown 中将包含全新的五大健康维度数据（心脏基础与隐性运动、睡眠架构与微觉醒、体成分SMI比例、久坐波段、基于心率的卡路里消耗估算等）。
- **数据深度关联（核心指令）**：**必须**结合饮食板块的摄入（Calories in）与外部提取的总能量消耗（Calories out = TDEE）分析当天的能量盈亏。**必须**寻找指标间的隐秘联系，例如：“检测到深睡比例下降，这可能与你昨晚高碳水饮食有关” 或 “今日长达4个波段的3小时久坐，抵消了你傍晚因做家务产生的隐性有氧消耗”。绝不要机械罗列数据。
- **医学教练级别输出**：不要使用生硬的模版。使用具有专业温度的私人医生语气，主动指出健康亮点（如静息心率低、肌肉量维持得好）和隐患风险，并在末尾给出具体可执行的 1-2 条建议。
- **保留精华**：涵盖原有的热量评分等图表，并在其下端新增『多维健康深度洞察』文字板块。
