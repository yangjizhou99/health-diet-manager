...
仅在用户意图明确属于以下任一情况时，才允许使用纯饮食报告：
- 用户明确说了“饮食日报/饮食周报/饮食月报”
- 用户明确说了“只看吃的”“不需要步数/睡眠/心率”等排除外部健康数据的话

若用户说的是“健康报告”“健康日报”“昨日健康总结”“帮我重生成昨天报告”这类默认包含整体健康语义的请求，**不得**路由到本节纯饮食报告。
...
为防止模型把“健康报告”误降级成“饮食报告”，必须按以下优先级执行：

1. 用户请求中只要出现“健康报告/健康日报/健康周报/健康月报/昨日健康报告/重生成昨日报告/总结昨天健康情况”等整体健康语义，**一律优先走 `generate-merged`**。
2. 只有当用户明确限定“只看饮食”或明确排除外部健康数据时，才允许走 `generate`。
3. 不允许模型自行把“报告”理解成“饮食报告”；若用户未明确限定，默认含外部健康数据。

### 5.3.2 综合报告失败时的响应规则（必须遵守）

若 `generate-merged` 因以下原因失败：
- 外部健康缓存不存在
- 自动同步失败
- `strict_real_data` 启用后发现缺少真实数据
- 外部缓存存在但一致性校验失败

则必须：

1. 明确告诉用户“本次综合健康报告未成功生成”以及失败原因。
2. 告诉用户下一步需要做什么，例如重新同步、补齐授权、指定本地回退目录、或稍后重试。
3. **禁止**在未获得用户明确许可的情况下，自动退回输出纯饮食报告。
4. **禁止**把纯饮食摘要包装成“健康报告”“昨日健康报告”“综合报告”。

允许的替代说法示例：

```markdown
昨日综合健康报告未成功生成。
原因：缺少 2026-03-07 的外部健康缓存，且当前 strict_real_data=true，不允许降级为估算或纯饮食版健康报告。
下一步：请先执行外部数据同步，再重新生成综合健康日报。
```
...
### 5.3.3 综合报告最小输出标准（必须完整）

为防止执行端只输出“几个指标 + 两三句总结”的简版内容，`generate-merged` 成功后，对用户展示的综合报告至少必须覆盖以下板块：
...
8. **体成分或体重相关指标**（若当期有数据）
9. **关键异常/关键亮点**
10. **后续建议**（日报 2-3 条，周报 3-4 条，月报 4-5 条）

若某板块确实没有数据，也必须显式写出“无数据/未同步/未记录”，不得直接省略整个板块。
...
- 报告类型：综合健康日报
- 覆盖日期：2026-03-07
- 数据来源：daily_log.json + health_cache_day_2026-03-07.json
- 生成口径：饮食记录与外部健康数据联合分析
```
...
如果缺少这部分，用户无法判断结论是否可靠，视为报告不完整。

#### C) 饮食分析（必须有）

至少包含：
- 热量、蛋白质、碳水、脂肪、纤维的摄入值、目标值、达标率
- 高频食物或主要饮食构成
- 饮食均衡评分
- 至少 1 条饮食层面的关键观察
...
至少包含：
- 静息心率或心率基线
- 推测运动次数 / Zone2+ 时长 / 有氧时长之一
- 对心血管负荷或运动恢复的简要解释

禁止只写“静息心率优秀”这种无上下文判断。
...
若存在估算值，则必须同时给出：
- 区间值（low/high）或明确标记为估算
- 置信度标签

禁止在低置信度时用绝对确定语气下结论。

#### H) 体成分或体重相关指标（有数据则必须展示）

如缓存中存在体重、体脂率、骨骼肌、SMI、肌肉脂肪比等字段，至少展示其中可用的主要指标，并指出这些指标是“当期最新值”还是“周期末值”。
...
允许示例：
- 热量缺口较大：摄入 1962 kcal，TDEE 2928 kcal，单日差值 -966 kcal
- 久坐暴露偏高：出现 4 个长时久坐 block
- 恢复较好：睡眠 8.4 小时，深睡比例 28.7%
...
如果最终对用户展示的内容少于上述主体板块中的 7 个，视为“综合报告过度简化”，执行失败。

### 5.3.4 禁止的简化输出模式

以下输出虽然看起来像“综合报告”，但都视为不合格：

1. 只有饮食表格 + 步数 + 两三句总结。
2. 只有若干数值，没有数据完整度与可信度说明。
3. 只写结论，不写支撑这些结论的指标。
4. 有能耗结论，但不说明是否来自估算或真实记录。
5. 体成分、睡眠、心率明明有数据，却被整段省略。
...
3. 禁止在用户请求“健康报告/健康日报/综合报告”时，未经明确说明就自动降级为纯饮食报告。
4. 禁止把纯饮食报告、饮食摘要或营养评分包装成“健康报告”对外输出。
5. 禁止在综合报告生成失败后省略失败原因，直接给出看似完整的简化版结论。
